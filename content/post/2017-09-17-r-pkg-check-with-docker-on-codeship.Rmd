...
The so called [recommended packages](https://stackoverflow.com/questions/9700799/what-is-difference-between-r-base-and-r-recommended-packages) are usually installed alongside R and not necessarily show up in the packrat `Requires` field which can cause surprises as `rocker/verse` does not installs them. So far we needed MASS for ggplot2 and lattice for zoo. We installed these before packrat bootstrap with `RUN R -e "install.packages("MASS")`.
...
We will think about installing recommended packages always or part of the base Docker image.
